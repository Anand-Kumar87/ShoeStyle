...
export default function EditProduct() {
    ...
    // Sync selected categories when categories or formData change
    useEffect(() => {
        if (categories.length > 0 && (formData.category || formData.categoryId)) {
            const rawCat = formData.category || '';
            const names = rawCat.split(',').map((s: string) => s.trim().toLowerCase());
            const matchedIds = categories
                .filter(c => names.includes(c.name.toLowerCase()) || c.id === formData.categoryId)
                .map(c => c.id);
            if (formData.categoryId && !matchedIds.includes(formData.categoryId)) {
                matchedIds.push(formData.categoryId);
            }
            if (matchedIds.length > 0) {
                setSelectedCategoryIds(prev => prev.length > 0 ? prev : matchedIds);
            }
        }
    ...
    const fetchProduct = async () => {
        try {
            ...
            setFormData({
                ...
                // Keep raw URLs in text fields for fallback
    ...
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-[#080d1a]"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div></div>;
    }
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-[#080d1a] pb-20 text-slate-900 dark:text-white">
            ...
            <div className="sticky top-0 z-40 bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Edit Product</h1>
                </div>
                <div className="flex items-center gap-3">
                    <Link href="/admin/products"><button className="px-5 py-2.5 font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors">Cancel</button></Link>
                    <button onClick={handleSubmit} disabled={saving} className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-blue-700 shadow-lg shadow-blue-200 dark:shadow-none transition-all disabled:opacity-50">
            ...
            <div className="max-w-6xl mx-auto px-6 mt-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {/* 🔥 PREMIUM MEDIA UPLOAD SECTION (Paste + Preview) */}
                        <div className="bg-white dark:bg-[#0f172a] p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                        ...
                        <div className="bg-white dark:bg-[#0f172a] p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="sizes" value={formData.sizes} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 p-3.5 rounded-xl font-medium focus:ring-2 focus:ring-blue-600 focus:bg-white dark:focus:bg-slate-900 outline-none transition-all" />
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="colors" value={formData.colors} onChange={handleChange} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 p-3.5 rounded-xl font-medium focus:ring-2 focus:ring-blue-600 focus:bg-white dark:focus:bg-slate-900 outline-none transition-all" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-8">
                        <div className="bg-white dark:bg-[#0f172a] p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2">Price (₹) <span className="text-red-500">*</span></label>
