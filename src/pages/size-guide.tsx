...
import { DEFAULT_SIZE_CHARTS } from './api/size-guide';

interface SizeGuideProps {
  initialCharts?: typeof DEFAULT_SIZE_CHARTS;
}

export default function SizeGuide({ initialCharts }: SizeGuideProps) {
  const sizeCharts = initialCharts || DEFAULT_SIZE_CHARTS;
...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    ...
    let charts = DEFAULT_SIZE_CHARTS;
    if (setting?.value) {
      try {
    ...
    console.error('Size guide getServerSideProps error:', error);
    return {
      props: {
        initialCharts: DEFAULT_SIZE_CHARTS,
      },
    };
  }
};